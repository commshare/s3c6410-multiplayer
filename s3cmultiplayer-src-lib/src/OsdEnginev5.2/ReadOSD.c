...
static float sub_delay;
static float sub_fps;
...

{
  ...
  return strlcpy (dst - len, src, dst_sz - len);
...
subtitle *sub_read_line_sami(FILE *fd, subtitle *current) {
  ...
  do {
    ...
    case 1: 
	    for  (; *s == ' ' || *s == '\t'; s++); 
	    if (*s == '\0') break;
	    if (*s != '<') { state = 3; p = text; continue; } 
	    s++;
	    if (*s == 'P' || *s == 'p') { s++; state = 2; continue; } 
	    for (; *s != '>' && *s != '\0'; s++); 
	    if (s == '\0')
	      break;
	    s++;
...
subtitle *sub_read_line_microdvd(FILE *fd,subtitle *current) {
  ...
  while ((next =sub_readtext (next, &(current->text[i])))) {
    if (current->text[i]==ERR) {return NULL;}
    i++;
    if (i>=SUB_MAX_TEXT) { puts("Too many lines in a subtitle\n");current->lines=i;return current;}
    }
  current->lines= ++i;
...
subtitle *sub_read_line_subrip(FILE *fd, subtitle *current) {
	...
	for (current->lines=1; current->lines < SUB_MAX_TEXT; current->lines++) {
	    ...
	    if (!current->text[current->lines-1]) return NULL;
...
void	subcp_open (FILE *enca_fd)
{
	...
	if (sub_cp){
		...
		if ((icdsc = iconv_open (tocp, cp_tmp)) != (iconv_t)(-1)){
			puts("SUB: opened iconv descriptor.\n");
			sub_utf8 = 2;
		} else
			puts("SUB: error opening iconv descriptor.\n");
#ifdef HAVE_ENCA
...
void	subcp_close (void)
{
	if (icdsc != (iconv_t)(-1)){
		...
		icdsc = (iconv_t)(-1);
	   	puts("SUB: closed iconv descriptor.\n");
	}
}
...
subtitle* subcp_recode (subtitle *sub)
{
	...
	while (l){
		...
		if (iconv(icdsc, &ip, &ileft,
			  &op, &oleft) == (size_t)(-1)) {
			puts("SUB: error recoding line (1).\n");
			l++;
			break;
		}
		if (!(ot = (char *)malloc(op - icbuffer + 1))){
			puts("SUB: error allocating mem.\n");
			l++;
		   	break;
		}
		*op='\0' ;
...
subtitle* subcp_recode1 (subtitle *sub)
{
  ...
  while (l){
     ...
     if (iconv(icdsc, &ip, &ileft,
	      &op, &oleft) == (size_t)(-1)) {
	puts("SUB: error recoding line (2).\n");
	return sub;
     }
     *op='\0' ;
  }
...
{
  FriBidiChar logical[LINE_LEN+1], visual[LINE_LEN+1]; 
  ...
  while (l) {
    ...
    if(len > LINE_LEN) {
      puts("SUB: sub->text is longer than LINE_LEN.\n");
      l++;
      break;
    }
...
static void adjust_subs_time(subtitle* sub, float subtime, float fps, int block,
	...
	unsigned long overlap = (unsigned long)((sub_uses_time ? 100 : fps) / 5); 
	...
	if (n) fprintf(stdout,"SUB: Adjusted %d subtitle(s).\n", n);
}
...
void* guess_cp(FILE *fd, char *preferred_language, char *fallback)
{
    ...
    puts( "ENCA supported languages: ");
    for (i = 0; i < langcnt; i++) {
	fprintf(stdout,"%s ", languages[i]);
    }
    puts("\n");
	...
	if (tmp && encoding.charset != ENCA_CS_UNKNOWN) {
	    ...
	    fprintf("ENCA detected charset: %s\n", tmp);
	}
	...
	fprintf("ENCA detection failed: fallback to %s\n", fallback);
    }
...
sub_data* sub_read_file (char *filename, float fps) {
    ...
    if (sub_format==SUB_INVALID) {puts("SUB: Could not determine file format\n");return NULL;}
    srp=sr+sub_format;
    fprintf(stdout,"SUB: Detected subtitle file format: %s\n", srp->name);
...
#endif	
        if(sub==ERR) ++sub_errs; else ++sub_num; 
    }
    ...
    fprintf(stdout,"SUB: Read %i subtitles", sub_num);
    if (sub_errs) fprintf(stdout,", %i bad line(s).\n", sub_errs);
    else 	  puts(".\n");
...
if ((suboverlap_enabled == 2) ||
	...
	do {
		...
		if ((first[sub_first + j].start <= local_end) && (first[sub_first + j].end > local_start)) {
			...
			puts( "SUB: we could not find a suitable position for an overlapping subtitle\n");
...
void dump_srt(sub_data* subd, float fps){
	...
	if (!subd->sub_uses_time)
	    temp = (unsigned long)(temp * 100 / sub_fps);
	temp -= (unsigned long)(sub_delay * 100);
    ...
    fclose(fd);
    puts("SUB: Subtitles dumped in \'dumpsub.srt\'.\n");
}
...
void dump_mpsub(sub_data* subd, float fps){
	...
	fclose (fd);
	puts("SUB: Subtitles dumped in \'dump.mpsub\'.\n");
}
...
void dump_microdvd(sub_data* subd, float fps) {
    ...
    delay = (int)(sub_delay * sub_fps);
    for (i = 0; i < subd->sub_num; ++i) {
	int j, start, end;
	start = subs[i].start;
	end = subs[i].end;
	if (subd->sub_uses_time) {
	    start = (int)(start * sub_fps / 100 );
	    end = (int)(end * sub_fps / 100);
	}
	else {
	    start = (int)(start * sub_fps / fps);
	    end = (int)(end * sub_fps / fps);
	}
	start -= delay;
	end -= delay;
    ...
    fclose(fd);
    puts("SUB: Subtitles dumped in \'dumpsub.txt\'.\n");
}
...
void dump_jacosub(sub_data* subd, float fps) {
    ...
    fclose(fd);
    puts("SUB: Subtitles dumped in \'dumpsub.js\'.\n");
}
...
void dump_sami(sub_data* subd, float fps) {
    ...
    fprintf(fd, "</BODY>\n"
		"</SAMI>\n");
    fclose(fd);
    puts("SUB: Subtitles dumped in \'dumpsub.smi\'.\n");
}
    ...
    for(i=0; i < subd->sub_num; i++)
     {
        ...
        temp_start = onesub->start;
        temp_end = onesub->end;

        if( pts>= temp_start && pts <= temp_end ) return onesub;
     }

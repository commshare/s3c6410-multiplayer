...
void* StartScale(uint8_t* src,int insize,int* outsize,unsigned int phyaddr,unsigned int *phyoutaddr)
{
	...
	if(phyaddr != 0)
	{
	      sparam.m_bparam.SrcFrmSt = phyaddr;	
		ioctl(sparam.m_bfileID, PP_SET_SRC_BUF_ADDR_PHY,&sparam.m_bparam);
	}
	else
	...
	if(phyoutaddr)
		*phyoutaddr = sparam.m_bparam.DstFrmSt;

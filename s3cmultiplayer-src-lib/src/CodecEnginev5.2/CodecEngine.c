...
static int InitCodec(int stream_index)
{
	...
	AVCodecContext *enc;
      ...
      if (enc->codec_type == AVMEDIA_TYPE_AUDIO) {
          if (enc->channels > 0) {
              enc->request_channels = enc->channels;//FFMIN(2, enc->channels);
          } else {
...
unsigned int VideoOutPyaddr()
{
	...
	if(Ocx.m_bHEnable && Ocx.m_bOpenHWHandle)
	{
		unsigned int YUVbuf[2];
		HwDecodeGetConfig(Ocx.m_bOpenHWHandle,SSBSIPMF_PHYADDR_FRAM_BUF,YUVbuf);
		
		addr = YUVbuf[0];	
	}
	else
		addr = CMMPhyAddr(Ocx.m_pHwVideoStreamBuffer);
